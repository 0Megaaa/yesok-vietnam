...
type SysUser struct {
	...
	CurrentTokenHash string     `json:"-" gorm:"size:64;index;comment:'当前有效后台登录token的sha256哈希，用于单点登录'"`
	TokenIssuedAt    *time.Time `json:"-" gorm:"comment:'当前token签发时间'"`
	TokenExpiresAt   *time.Time `json:"-" gorm:"comment:'当前token过期时间'"`
