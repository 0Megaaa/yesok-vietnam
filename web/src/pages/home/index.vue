...
<template>
  ...
  <!-- ── HERO BANNER ── -->
  <!-- Banner 图片通过 CSS 注入本地静态资源，搜索框移出 hero 置于正下方 -->
  <view class="hero"
    style="background-image: url('/static/img.png'); background-size: cover; background-position: center top; width: 100%; height: 260px; padding-top: 50px; border-radius: 0 0 24px 24px;">
    <view class="tb-main" style="padding: 0 20px; display: flex; justify-content: space-between; align-items: center;">
      <view class="logo" style="font-size: 20px; font-weight: 800; color: #fff;">Yesok <text style="color:var(--gd);">Vietnam</text></view>
  ...
  <!-- 搜索框：移出 hero，置于正下方，带负 margin 上移 -->
  <view style="padding: 0 20px; margin-top: -24px; position: relative; z-index: 10;">
    <view class="search-bar" style="background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); border-radius: 12px; padding: 12px 16px; display: flex; align-items: center; gap: 10px; box-shadow: var(--sh2);">
      <text>🔍</text>
  ...
  <!-- ── SERVICE CATEGORIES ── -->
  <view style="background:#fff;padding:4px 0 10px;margin-bottom:14px;box-shadow:0 1px 8px rgba(0,0,0,.04);">
    <view style="display:flex;justify-content:space-around;padding:8px 8px 0;">

      <view @click="goPage('services')"
        style="display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;width:54px;">
        <view style="width:48px;height:48px;border-radius:50%;background:#F0F6FF;display:flex;align-items:center;justify-content:center;font-size:22px;">✈️</view>
        <view style="font-size:11px;color:#1A2340;font-weight:500;text-align:center;">签证旅游</view>
      </view>

      <view @click="goPage('services')"
        style="display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;width:54px;">
        <view style="width:48px;height:48px;border-radius:50%;background:#F0F6FF;display:flex;align-items:center;justify-content:center;font-size:22px;">🏢</view>
        <view style="font-size:11px;color:#1A2340;font-weight:500;text-align:center;">房产投资</view>
      </view>

      <view @click="goPage('services')"
        style="display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;width:54px;">
        <view style="width:48px;height:48px;border-radius:50%;background:#F0F6FF;display:flex;align-items:center;justify-content:center;font-size:22px;">📋</view>
        <view style="font-size:11px;color:#1A2340;font-weight:500;text-align:center;">公司注册</view>
      </view>

      <view @click="goPage('services')"
        style="display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;width:54px;">
        <view style="width:48px;height:48px;border-radius:50%;background:#F0F6FF;display:flex;align-items:center;justify-content:center;font-size:22px;">🚗</view>
        <view style="font-size:11px;color:#1A2340;font-weight:500;text-align:center;">接送出行</view>
      </view>

      <view @click="goPage('services')"
        style="display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;width:54px;">
        <view style="width:48px;height:48px;border-radius:50%;background:#F0F6FF;display:flex;align-items:center;justify-content:center;font-size:22px;">🗣️</view>
        <view style="font-size:11px;color:#1A2340;font-weight:500;text-align:center;">翻译服务</view>
      </view>

      <view @click="goPage('services')"
        style="display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;width:54px;">
        <view style="width:48px;height:48px;border-radius:50%;background:#F0F6FF;display:flex;align-items:center;justify-content:center;font-size:20px;color:#9AA3B5;font-weight:700;">···</view>
        <view style="font-size:11px;color:#1A2340;font-weight:500;text-align:center;">更多服务</view>
      </view>
  ...
  <view style="background:#fff;margin-bottom:14px;padding:14px 16px 4px;box-shadow:0 1px 8px rgba(0,0,0,.04);">
    <view style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
      <view style="font-size:16px;font-weight:700;color:#1A2340;">热门服务</view>
      <view @click="goPage('services')"
        style="font-size:12px;color:#9AA3B5;cursor:pointer;display:flex;align-items:center;gap:2px;">
        全部服务
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#9AA3B5" stroke-width="2">
    ...
    <!-- 横向滚动卡片列表 -->
    <scroll-view class="hot-scroll" scroll-x style="white-space: nowrap; padding: 10px 16px;">
      <view
        v-for="svc in SVCS_DATA"
        :key="svc.id"
        class="hc-card"
        style="display: inline-block; width: 280px; margin-right: 12px; background: #fff; border-radius: 16px; padding: 16px; box-shadow: var(--sh); vertical-align: top;">

        <view style="display: flex; gap: 12px;">

          <!-- 左侧图标 -->
          <view class="hc-icon" :style="{ background: svc.bg }"
            style="width: 48px; height: 48px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 24px; flex-shrink: 0;">
            {{ svc.icon }}
          </view>

          <!-- 右侧信息 -->
          <view class="hc-info" style="flex: 1; min-width: 0;">
            <view class="hc-name" :style="{ color: svc.color }"
              style="font-size: 15px; font-weight: 700; margin-bottom: 4px;">
              {{ svc.name }}
            </view>
            <view class="hc-sub" style="font-size: 12px; color: var(--tx2); margin-bottom: 8px;">
              {{ svc.sub }}
            </view>

            <!-- 标签 -->
            <view class="hc-tags" style="display: flex; gap: 6px; margin-bottom: 12px;">
              <text
                v-for="tag in svc.tags"
                :key="tag"
                class="hc-tag"
                style="padding: 2px 6px; background: var(--gy); border-radius: 4px; font-size: 10px; color: var(--tx3);">
                {{ tag }}
              </text>
            </view>

            <!-- 底部：价格 + 去咨询按钮 -->
            <view class="hc-bot" style="display: flex; justify-content: space-between; align-items: center; border-top: 1px dashed #eee; padding-top: 10px;">
              <view class="hc-price" style="font-size: 16px; font-weight: 800; color: var(--tx);">
                {{ svc.price }}<text style="font-size: 10px; font-weight: 400; color: var(--tx3);">/{{ svc.unit }}</text>
              </view>
              <button
                class="hc-btn"
                :style="{ background: svc.color }"
                style="margin: 0; padding: 0 16px; height: 28px; line-height: 28px; border-radius: 14px; color: #fff; font-size: 12px; font-weight: 600; border: none;"
  ...
  <view style="padding: 0 16px; margin-bottom: 24px;">
    <view style="background: #fff; border-radius: 16px; box-shadow: var(--sh); overflow: hidden;">
      <view
        ...
        :key="n.id"
        class="sr-item">
        <view class="sr-type news">📰</view>
        <view style="flex: 1; min-width: 0;">
          <view class="sr-name" style="white-space: normal; line-height: 1.4; margin-bottom: 4px;">
            <text v-if="n.top" style="color: var(--rd);">📌 </text>{{ n.title }}
          </view>
          <view class="sr-sub">{{ n.tag }} · {{ n.source }}</view>
        </view>
        <view class="sr-badge" style="background: var(--gy2); color: var(--tx3);">
          {{ n.date }}
        </view>
      </view>
    </view>
  ...
  <!-- ── 为什么选择 Yesok ── -->
  <view style="margin: 20px 16px; padding: 20px; background: #fff; border-radius: 16px; box-shadow: var(--sh);">
  ...
  <view style="border-top:1px solid var(--br);padding-top:4px;">
    <view style="padding:14px 16px 10px;">
      <view style="font-size:16px;font-weight:700;color:#1A2340;">📋 订单动态</view>
    </view>
    ...
    <view class="ord-list">
      ...
      <template v-else>
        ...
        >
          <view class="oc-top">
            <view>
              <view class="oc-svc">{{ order.icon || '📋' }} {{ order.serviceName || order.svc }}</view>
          ...
          <view style="margin:9px 0;">
            <view class="oc-pbar">
              <view class="oc-pfill" :style="{ width: getStatus(order.status || order.sk).p + '%' }"></view>
            </view>
            <view class="oc-plbl">
          ...
          <view class="oc-bot">
            ...
            <view style="font-size:11px;color:var(--tx3);">{{ order.price || order.pr || '—' }}</view>
          </view>
        </view>
      </template>
    </view>
