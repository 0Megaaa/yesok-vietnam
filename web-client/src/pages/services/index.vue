...
<template>
  <view class="services-page">
    <view class="top">
      ...
      <text class="sub">所有服务、价格与配置均来自后台 sys_services</text>
      <view class="search"><input v-model="keyword" placeholder="搜索服务"/></view>
    </view>
    <view v-if="loading" class="empty">正在读取后台服务配置...</view>
    <view v-else class="list">
      <view v-for="service in filteredServices" :key="service.id" class="card">
        <view class="icon">{{ service.icon || '🌴' }}</view>
        <view class="main">
          <text class="name">{{ service.display_name }}</text>
          <text class="desc">{{ service.description }}</text>
          <view class="bottom">
            <text class="price">{{ service.price_text }}/{{ service.unit || '次' }}</text>
            <view class="consult-button" @click="consultService(service)">去咨询</view>
          </view>
        </view>
      </view>
    </view>
...
.search {
  ...
  box-shadow: 0 16px 42px rgba(0, 77, 64, .08);
...
.card {
  ...
  border-radius: 28px;
  background: #fff;
  box-shadow: 0 16px 46px rgba(0, 77, 64, .07);
}
...
.icon {
  ...
  height: 54px;
  border-radius: 20px;
  background: linear-gradient(135deg, rgba(0, 77, 64, .1), rgba(245, 217, 143, .3));
  font-size: 26px;
}
